...
class AtmMutil(ATM):
    ...
    def change_passwd(self):
        ...
        new_passwd_02 = input('请再次输入新的密码：')

        # 查询账户信息
        account = self.balance()
        if account['passwd'] != old_passwd:
            return False, '旧密码错误'
        elif new_passwd != new_passwd_02:
    ...
    def main(self):
        ...
        check_dict = {'1':self.take_money, '2':self.save_money ,'3':self.balance, '4':self.exit, '5':self.change_passwd}
        info = """
        ####################
            可选择操作：
            1、取钱
